...
5. **前端模块不要互相 import**：功能模块从 `shared.mjs` 取状态，通过 `setRenderHooks` / `onBrushChange` / `onLangChange` / `devEvents` 通信。唯一例外链：`ring → picker → brush → cursor`，`i18n ← settings ← devtools`，`connection → loader`。
...
| `src/dev-api.ts` | `doc/protocol.md`、密码来源、token、失败锁定、批量上限、`range` 颜色字段、导入导出密码 |
